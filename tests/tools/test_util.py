from qebil.tools.util import load_project_file, retrieve_ftp_file, check_download_integrity, scrape_ebi_ids, parse_document
import unittest
from os import path
...
class utilTest(unittest.TestCase):
    ...
    def test_load_project_file(self):
        ...
        test_project_file_no_header = _test_support_dir + '/test_project_file_no_header.tsv'
        test_load_project_list = load_project_file(test_project_file)
        test_load_project_list_no_header = load_project_file(test_project_file_no_header)

        self.assertEqual(len(test_load_project_list),2)
        self.assertEqual(len(test_load_project_list_no_header),2)
        self.assertEqual(test_load_project_list, test_load_project_list_no_header)

    def test_retrieve_ftp_file(self):
        test_ftp_path = 'ftp.sra.ebi.ac.uk/vol1/fastq/SRR126/080/SRR12672280/SRR12672280_1.fastq.gz'
        test_local_fastq_path = _test_output_dir + '/SRR12672280_1.fastq.gz'        
        test_checksum = '5c1da3b86d2bbb0d09e1f05cef0107f2'
        test_corrupt_fastq_path = _test_support_dir + '/corrupt_fastq1.fastq.gz'
        test_overwrite = True
        
        test_checksum_1 = retrieve_ftp_file(test_ftp_path, test_local_fastq_path, test_checksum)
        ...
        test_checksum_2 = retrieve_ftp_file(test_ftp_path, test_local_fastq_path, test_checksum)
        self.assertTrue(path.isfile(test_local_fastq_path))
        self.assertEqual(test_checksum, test_checksum_2)

    def test_check_download_integrity(self):
        test_local_fastq_path = _test_support_dir + '/SRR11192680_1.fastq.gz'
        expected_md5 = '5c1da3b86d2bbb0d09e1f05cef0107f2'
        self.assertTrue(path.isfile(test_local_fastq_path))
        md5checksum_res = check_download_integrity(test_local_fastq_path,expected_md5)
        self.assertEqual(expected_md5, md5checksum_res)

    def test_parse_document(self):
        tokens_html = parse_document('https://www.nature.com/articles/s41598-020-60564-8')
        tokens_pdf = parse_document('https://www.nature.com/articles/s41598-021-83922-6.pdf')
        
        self.assertEqual(len(tokens_html),77959)
        self.assertEqual(len(tokens_pdf),15771)
    
    def scrape_ebi_ids(self):
        test_paths = ['https://www.nature.com/articles/s41598-021-83922-6.pdf',
