...
from qebil.fetch import fetch_ebi_info, fetch_fastq_files, fetch_fastqs, fetch_ebi_metadata
...
class FetchTest(unittest.TestCase):
    ...
    def test_fetch_fastq_files(self):
        ...
        test_raw_reads = fetch_fastq_files(test_run_prefix,test_read_dict, _test_output_dir)
    ...
    def test_fetch_fastqs(self):
        ...
        test_study_df = pd.read_csv(_test_support_dir + '/test_study.EBI_metadata.tsv',
                                    sep='\t', header=0, index_col =0).head(1)
        test_study = Study(test_study_df)
        test_expected_files = ['SAMN16049500.SRR12672280.R1.ebi.fastq.gz','SAMN16049500.SRR12672280.R2.ebi.fastq.gz']
        test_expected_reads = '5740751'
        test_result_md = fetch_fastqs(test_study, _test_output_dir)
        print(test_result_md)
        test_result_files = [ f.split('/')[-1] for f in sorted(glob.glob(_test_output_dir + '/*.fastq.gz'))]
        self.assertEqual(str(test_result_md["qiita_raw_reads"].sum()),test_expected_reads)
    ...
    def test_fetch_ebi_info_study(self):
        ...
        self.assertEquals(test_xml_dict,test_study_dict)        
        
    def test_fetch_ebi_info_sample(self):
        accession='SRS7392559'
        test_sample_dict = OrderedDict([('SAMPLE_SET',
              OrderedDict([('SAMPLE',
                            OrderedDict([('@accession', 'SRS7392559'),
                                         ('@alias', 'MH001880_V1'),
    ...
    def test_fetch_ebi_info_expt(self):
        ...
        test_xml_dict = fetch_ebi_info(accession)
        test_expt_dict = OrderedDict([('EXPERIMENT_SET',
              OrderedDict([('EXPERIMENT',
                            OrderedDict([('@accession', 'SRX9152556'),
                                         ('@alias', 'MH001880_V1'),
                                         ...
                                         ('EXPERIMENT_ATTRIBUTES',
                                          OrderedDict([('EXPERIMENT_ATTRIBUTE',
                                                        [OrderedDict([('TAG',
                                                                       'ENA-SPOT-COUNT'),
                                                                      ('VALUE',
                                                                       '5740751')]),
    ...
    def test_fetch_ebi_metadata(self):
        accession='SRP283872'
        test_limited_fields = ["run_accession", "sample_accession", "library_name"]
        test_study_valid_tsv = _test_support_dir + '/test_study_basic.tsv'
        test_expected_df =pd.read_csv(test_study_valid_tsv,header = 0,sep= '\t', dtype=str)
        test_result_df = fetch_ebi_metadata(accession)
        test_limited_result_df = fetch_ebi_metadata(accession, test_limited_fields)
        print(test_expected_df.columns)
        print(test_result_df.columns)
        assert_frame_equal(test_expected_df,test_result_df)
        self.assertEqual(list(test_limited_result_df.columns),test_limited_fields)
