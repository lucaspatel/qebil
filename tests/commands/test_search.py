...
class TestSearch(unittest.TestCase):
    ...
    def test_keyword_search(self):
        query = "PRJNA660883"
        test_selection_dict = {}
        test_selection_dict['library_strategy'] = ['amplicon', 'other', 'wgs', 'rna-seq']
        test_selection_dict['instrument_platform'] = ['illumina']
        test_selection_dict['library_selection'] = ['random', 'pcr', 'random pcr',
                                                'cdna_randompriming', 'inverse rrna',
                                                'inverse rrna selection', 'other',
                                                'unspecified','size fractionation']            
        test_selection_dict['library_source'] = ["genomic", "genomic single cell",
                                                 "transcriptomic",
                                                 "transcriptomic single cell",
                                                 "metagenomic", "metatranscriptomic",
                                                 "viral rna", "other",]   
        test_summary = ['study_title', 'scientific_name', 'library_source',
                            'library_strategy', 'library_selection',
                            'instrument_platform']
        test_result_0 = keyword_search(query, filter_dict={},
                                     summarize=test_summary)
        test_result = keyword_search(query, filter_dict=test_selection_dict,
                                     summarize=test_summary)
        self.assertEqual(len(test_result_0),1)
        self.assertEqual(len(test_result),1)
        self.assertCountEqual(test_result.columns,["study_id", "samples"] + test_summary)
    ...
    def test_search_ebi(self):        
        ...
        search_args = ['ebi','--query',query,'--output-dir',output_dir,'--prefix',prefix]
        for st in strategy:
        ...
        test_result = pd.read_table(output_dir + '/' + prefix + '_EBI_search_results.tsv')
        self.assertEqual(len(test_result),1)
        self.assertCountEqual(test_result.columns,["study_id", "samples"] + summarize)
    ...
    def test_search_ebi_no_summary(self):
        ...
        test_result = pd.read_table(output_dir + prefix + '_EBI_search_results.tsv')
        self.assertEqual(len(test_result),1)
        self.assertCountEqual(test_result.columns,["study_id", 'samples','study_title','scientific_name', 'library_source',
                            'library_strategy', 'library_selection',
                            'instrument_platform'])
