from qebil.commands.fetch import fetch_remote_studies, check_existing_metadata, fetch
import unittest
...
class fetchTest(unittest.TestCase):
    ...
    def test_fetch_remote_studies(self):
        ...
        test_study_list = ['PRJNA660883','SRP283872']  # note, these should resolve to the same study
        ...
        result_dict = fetch_remote_studies(test_study_list, test_full_details,
                                           test_max_samples, test_random_subsample)
        self.assertEqual(result_dict[test_study_list[0]].study_id,test_study_list[0])
        self.assertEqual(len(result_dict[test_study_list[0]].metadata),36) 
        self.assertEqual(result_dict[test_study_list[1]].study_id,test_study_list[1])
        self.assertEqual(len(result_dict[test_study_list[1]].metadata),36) 
        self.assertNotEqual(result_dict[test_study_list[0]].study_id,test_study_list[1])
        
        results_dict_1 = fetch_remote_studies(test_study_list, test_full_details)
        self.assertCountEqual(result_dict.keys(),results_dict_1.keys())
        
        results_dict_2 = fetch_remote_studies(test_study_list, test_full_details,
                                           test_max_samples_num, test_random_subsample)
        results_dict_3 = fetch_remote_studies(test_study_list, test_full_details,
                                           test_max_samples_num_str, test_random_subsample)
        
        self.assertCountEqual(results_dict_2.keys(),results_dict_3.keys())
        self.assertRaises(fetch_remote_studies(test_study_list, test_full_details, test_random_subsample))
        self.assertRaises(fetch_remote_studies(test_study_list, test_full_details, max_samples=test_random_subsample))
        
        results_dict_4 = fetch_remote_studies(test_study_list, test_full_details,
                                           4, True)
    ...
    def test_fetch_project(self):
        ...
        test_metadata_file = _test_support_dir + '/test_study.EBI_metadata.tsv' 
        test_output_dir = _test_output_dir
        test_prefix = 'test_fetch_project'
        test_prep_max_num_str = '250'
        test_cpus_num_str = '4'
        test_max_samples_num_str = '2'
        fetch_args = ['--output-dir', test_output_dir,'--prefix', test_prefix, '--prep-max',test_prep_max_num_str,'--cpus',test_cpus_num_str,
                     '--max-samples',test_max_samples_num_str]
        strategy = ['amplicon', 'other', 'wgs', 'rna-seq']
        platform = ['illumina']
        selection = ['random', 'pcr', '"random pcr"', 'cdna_randompriming', '"inverse rrna"',
                                                '"inverse rrna selection"', 'other',
                                                'unspecified','"size fractionation"']            
        source = ['genomic', '"genomic single cell"',
                                                 'transcriptomic',
        ...
        fetch_file_args = ['project','--project-file',test_project_file] + fetch_args
        fetch_md_args = ['project','--metadata-file',test_metadata_file] + fetch_args
        ...
        test_metadata_base = _test_support_dir + '/SRP116878_sample_info.EBI_metadata.tsv'
        test_merge_metadata = _test_support_dir + '/test_merger_metadata.tsv'
        test_merge_col = 'library_name'
        fetch_augment_args = ['project','--metadata-file',test_metadata_base] + fetch_proj_args + ['--add_metadata_file',test_merge_metadata,'--merge_column',test_merge_col]        
        
        
        # additional valid configurations to test
        test_quiet_yes = '--quiet'
        test_qiita_yes = '--qiita'     
        test_qiita_no = '--raw'
