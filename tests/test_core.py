...
class StudyTest(unittest.TestCase):
    ...
    md_dict_empty = {}
    md_dict_basic = {'study_id':['TESTING123'],'study_accession':['PRJXXXX'],'sample_name':['sample1']}
    md_dict_invalid_sample = {'study_id':['TESTING123'],
    ...
    test_fields_defaults = ["library_name", "secondary_sample_accession", "run_accession", 
                            "experiment_accession", "fastq_ftp", "library_source", 
                            "instrument_platform", "submitted_format", "library_strategy", 
                            "library_layout", "tax_id", "scientific_name", 
                            "instrument_model", "library_selection", "center_name", 
                            "experiment_title", "study_title", "study_alias", 
                            "experiment_alias", "sample_alias",  "sample_title", 
                            "fastq_md5"]
    ...
    def test_study_creation_from_dict(self):
        for m in self.md_dict_list:
            ...
            assert_frame_equal(study_from_dict.metadata, pd.DataFrame.from_dict(m))
    """
    ...
    def test_study_creation_from_dataframe(self):
        for m in self.md_dict_list:
            ...
            assert_frame_equal(study_from_dataframe.metadata, from_dict_df)
    
    def test_study_creation_from_remote_valid(self):
        valid_study = Study.from_remote(self.test_study_valid_id, full_details=True, max_samples = 1)
        test_df = pd.read_csv(self.test_study_valid_tsv,header = 0,sep= '\t', dtype=str)
        test_df['sample_name'] = test_df['sample_accession']
        test_df = test_df.set_index('sample_name')
        #print(valid_study.metadata.columns)
        assert_frame_equal(valid_study.metadata,test_df)
        self.assertEqual(valid_study.study_id,self.test_study_valid_id)
                
    def test_study_creation_from_remote_valid_id_invalid_fields(self):
        test_df = pd.read_csv(self.test_study_valid_tsv,header = 0,sep= '\t', dtype=str)
        valid_study = Study.from_remote(self.test_study_valid_id, fields = self.test_fields_invalid)
        assert_frame_not_equal(valid_study.metadata,test_df)
        self.assertEqual(valid_study.study_id,self.test_study_valid_id)
    ...
    def test_populate_sample_names(self):
        ...
        # TODO: add test to ensure name resolution when sample_name is same for all samples
        # need to find study ID with this issue to use
  

    def test_populate_sample_info(self):
        test_study = Study.from_remote(self.test_study_valid_id, full_details = True, max_samples =1)
        test_df = pd.read_csv(self.test_study_valid_tsv,header = 0,sep= '\t', dtype=str)
    ...
    def test_populate_expt_info_and_preps(self):
        ...
        test_df = pd.read_csv(self.test_prep_valid_tsv, header = 0, sep= '\t', dtype=str, index_col = 0)
        prep_cols = test_df.columns
        prep_subset = test_study.metadata[prep_cols]
        assert_frame_equal(prep_subset,test_df)
        
    def test_filter_samples(self):
        test_study = Study.from_remote(self.test_study_mixed, full_details = True)
    ...
    def test_populate_details(self):
        # Note, this is almost identical to test_fetch_ebi_info_study, so maybe refactor
        # to check for the log messages?
        accession='SRP283872'
        test_study_dict = OrderedDict([('STUDY_SET',
              OrderedDict([('STUDY',
                            OrderedDict([('@accession', 'SRP283872'),
                                         ('@alias', 'PRJNA660883'),
